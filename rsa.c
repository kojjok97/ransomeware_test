...
void cryption(char * buffer,int n,int k,char * filename){
	...
	for(int i =0;i<strlen(buffer);i++){
		...
		enbuffer[i] = (int)mod(buffer[i+1],n,k);
		sprintf(enbu,"%d",enbuffer[i]);
		fprintf(enfp,"%s\n",enbu);
	}
		fclose(enfp);
...
int * ma(){
	...
	int * enbuffer = (int*)malloc(sizeof(int)*20000);
...
//완성
//복호화 모드 만들면 끝날듯
//기왕이면 .txt로 저장하지말고 다른 확장자로 저장하는 것도 나쁘지 않은듯 